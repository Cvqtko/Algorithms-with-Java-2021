...
public class Exercise_02 {
	public static void main(String[] args) throws IOException {
		...
		int[] singleShipTime = Arrays.stream(reader.readLine().split("\\s+")).mapToInt(Integer::parseInt).toArray();

		int[] pairShipTime = Arrays.stream(reader.readLine().split("\\s+")).mapToInt(Integer::parseInt).toArray();
		...
		for (int i = dp.length - 1; i > 0; i--) {
			...
			if (timeDiffForLatestTwo == singleShipTime[i - 1]) {
				...
				out.add(String.format("Pair of %d and %d", i - 1, i));
				i--;
			}
		}
