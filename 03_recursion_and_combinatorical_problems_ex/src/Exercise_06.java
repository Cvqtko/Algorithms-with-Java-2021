...
public class Exercise_06 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		// Lambda works only with final or effectively final variables, so we use AtomicInteger for counter
	...
	private static void findAreas(int row, int col) {
		if (isOutOfBounds(row, col) || hasHitWall(row, col)) {
			return;
		}
		matrix[row][col] = 'V';
	...
	private static boolean hasHitWall(int row, int col) {
